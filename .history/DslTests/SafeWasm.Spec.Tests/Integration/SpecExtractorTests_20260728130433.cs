    ...
    public sealed class SpecExtractorTests
    {
        ...
        public void ExtractFromWat_ExtractsGlobalInvariant()
        {
            ModuleSpec moduleSpec =
                SpecExtractor.ExtractFromWat(
                    Wat,
                    strict: true
                );

            SpecDirective invariant =
                Assert.Single(moduleSpec.GlobalInvariants);
        ...
        public void ExtractFromWat_AttachesRequiresToFunction()
        {
            ...
            Assert.True(
                moduleSpec.RequiresByFunc.ContainsKey(
                    "$acquire_request"
                )
            );

            SpecDirective requires = Assert.Single(
                moduleSpec.RequiresByFunc["$acquire_request"]
            );
            ...
            Assert.Equal(
                "(active_requests >= 0)",
                SpecAstPrinter.Print(requires.Expression)
            );
        }
        ...
        public void ExtractFromWat_AttachesEnsuresToFunction()
        {
            ...
            SpecDirective ensures = Assert.Single(
                moduleSpec.EnsuresByFunc["$acquire_request"]
            );
        ...
        public void ExtractFromWat_CollectsAssertAndAssume()
        {
            ...
            Assert.Equal(
                2,
                moduleSpec.InlineDirectives.Count
            );

            Assert.Equal(
                SpecDirectiveKind.Assert,
                moduleSpec.InlineDirectives[0].Kind
            );

            Assert.Equal(
                SpecDirectiveKind.Assume,
                moduleSpec.InlineDirectives[1].Kind
            );
        }
        ...
        public void ExtractFromWat_PreservesInlineOrder()
        {
            ...
            SpecDirective assertDirective =
                moduleSpec.InlineDirectives[0];

            SpecDirective assumeDirective =
                moduleSpec.InlineDirectives[1];

            Assert.True(
                assertDirective.Location.StartIndex
                <
                assumeDirective.Location.StartIndex
            );
        }
        ...
        public void ExtractFromWat_PreservesSourceLocations()
        {
            ...
            foreach (
                ...
                in moduleSpec.InlineDirectives)
            {
...
public void ExtractFromWat_ParsesDatatypeAccess()
{
    ...
    SpecDirective ensures = Assert.Single(
        moduleSpec.EnsuresByFunc["$get_result"]
    );
...
public void ExtractFromWat_AttachesContractsToCorrectFunctions()
{
    ...
    Assert.Single(
        moduleSpec.RequiresByFunc["$first"]
    );

    Assert.Single(
        moduleSpec.EnsuresByFunc["$first"]
    );

    Assert.Single(
        moduleSpec.RequiresByFunc["$second"]
    );

    Assert.Single(
        moduleSpec.EnsuresByFunc["$second"]
    );

    Assert.Equal(
        "(x >= 0)",
        SpecAstPrinter.Print(
            moduleSpec.RequiresByFunc["$first"][0].Expression
        )
    );
    ...
    Assert.Equal(
        "(y <= 10)",
        SpecAstPrinter.Print(
            moduleSpec.RequiresByFunc["$second"][0].Expression
        )
    );
}
