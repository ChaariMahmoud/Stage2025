...
class Program
{
   // const long MaxWatSizeBytes = 5L * 1024 * 1024; // 5 MB
    const int MaxInstructions = 20000000;
    const int TranslationTimeoutSec = 600;   // 10 min
const int SmtTimeoutSec = 2500;          // génération SMT
...
await File.WriteAllTextAsync(
    ...
    LimitText(
        $"SMT elapsed ms: {Math.Round(smtWatch.Elapsed.TotalMilliseconds, 2)}\n" +
        $"SMT file exists: {File.Exists(smtFile)}\n" +
        $"SMT file path: {smtFile}\n\n" +
        smt.Output
    )
);
...
{
    verify1000Status = "NOT_RUN_NO_SMT";
    verify2500Status = "NOT_RUN_NO_SMT";
    verify5000Status = "NOT_RUN_NO_SMT";
}
