...
var stat = CountInstructions(watFile);
                    ...
                    if (translationStatus is "PASS" or "PARTIAL")
                    {
                        ...
                        var smt = await RunBoogie(
                            boogieExe,
                            $"/proc:BoogieEntry_* /traceTimes /proverLog:{Quote(smtFile)} {Quote(boogieFile)}",
...
verify5000Status = await RunVerification(
    ...
    static void PrintResultRow(ContractResult r)
    {
        Console.WriteLine(
            $"Dataset={r.Dataset} | File={Path.GetFileName(r.WatFile)} | "
                ...
                + $"T1000={r.Verify1000Status} | T2500={r.Verify2500Status} | T5000={r.Verify5000Status} | "
                + $"Reason={r.Reason}"
        );
    }
    ...
    static void WriteCsv(string path, List<ContractResult> results)
    {
        ...
        sb.AppendLine(
            "dataset,wat_file,boogie_file,total_instructions,supported_instructions,unsupported_instructions,supported_percent,fully_supported_wat,translation_to_boogie,boogie_noverify,smt_generation,verify_1000s,verify_2500s,verify_5000s,translation_time_ms,unsupported_ops,reason"
        );
...
static void WriteSummary(string path, List<ContractResult> results)
{
    ...
    int errors = results.Count(r =>
        r.TranslationStatus == "ERROR" || r.TranslationStatus == "FAIL"
    );
    ...
    int smtAttempted = results.Count(r => r.SmtGenerationStatus != "NOT_RUN");
    int smtPass = results.Count(r => r.SmtGenerationStatus == "PASS");
    int smtTimeout = results.Count(r => r.SmtGenerationStatus.StartsWith("TIMEOUT"));
    int smtFail = results.Count(r =>
        r.SmtGenerationStatus != "PASS" &&
        r.SmtGenerationStatus != "NOT_RUN" &&
        !r.SmtGenerationStatus.StartsWith("TIMEOUT")
    );

    int v1000Attempted = results.Count(r => r.Verify1000Status != "NOT_RUN");
    int v2500Attempted = results.Count(r => r.Verify2500Status != "NOT_RUN");
    int v5000Attempted = results.Count(r => r.Verify5000Status != "NOT_RUN");

    int v1000Pass = results.Count(r => r.Verify1000Status == "PASS");
    int v2500Pass = results.Count(r => r.Verify2500Status == "PASS");
    int v5000Pass = results.Count(r => r.Verify5000Status == "PASS");

    int v1000Timeout = results.Count(r => r.Verify1000Status.StartsWith("TIMEOUT"));
    int v2500Timeout = results.Count(r => r.Verify2500Status.StartsWith("TIMEOUT"));
    int v5000Timeout = results.Count(r => r.Verify5000Status.StartsWith("TIMEOUT"));

    int v1000Fail = results.Count(r =>
        r.Verify1000Status != "PASS" &&
        r.Verify1000Status != "NOT_RUN" &&
        !r.Verify1000Status.StartsWith("TIMEOUT")
    );

    int v2500Fail = results.Count(r =>
        r.Verify2500Status != "PASS" &&
        r.Verify2500Status != "NOT_RUN" &&
        !r.Verify2500Status.StartsWith("TIMEOUT")
    );

    int v5000Fail = results.Count(r =>
        r.Verify5000Status != "PASS" &&
        r.Verify5000Status != "NOT_RUN" &&
        !r.Verify5000Status.StartsWith("TIMEOUT")
    );
    ...
    sb.AppendLine("---- Translation coverage ----");
    sb.AppendLine($"Fully supported WAT: {fullySupported} ({Percent(fullySupported, total)}% of total)");
    sb.AppendLine($"Boogie files generated: {boogieGenerated} ({Percent(boogieGenerated, total)}% of total)");
    sb.AppendLine($"  - PASS translations: {translatedPass} ({Percent(translatedPass, total)}% of total)");
    sb.AppendLine($"  - PARTIAL translations: {partial} ({Percent(partial, total)}% of total)");
    sb.AppendLine($"Translation timeout: {translationTimeout} ({Percent(translationTimeout, total)}% of total)");
    sb.AppendLine($"Translation errors/failures: {errors} ({Percent(errors, total)}% of total)");
    sb.AppendLine($"Out of memory skipped: {oom} ({Percent(oom, total)}% of total)");
    sb.AppendLine($"Skipped empty: {empty} ({Percent(empty, total)}% of total)");
    sb.AppendLine();

    sb.AppendLine("---- Boogie parsing/typechecking ----");
    sb.AppendLine($"Boogie /noVerify attempted: {noVerifyAttempted}");
    sb.AppendLine($"Boogie /noVerify PASS: {noVerifyPass} ({Percent(noVerifyPass, noVerifyAttempted)}% of attempted, {Percent(noVerifyPass, boogieGenerated)}% of Boogie files)");
    sb.AppendLine();

    sb.AppendLine("---- SMT generation ----");
    sb.AppendLine($"SMT generation attempted: {smtAttempted}");
    sb.AppendLine($"SMT generated: {smtPass} ({Percent(smtPass, smtAttempted)}% of attempted, {Percent(smtPass, boogieGenerated)}% of Boogie files)");
    sb.AppendLine($"SMT timeout: {smtTimeout} ({Percent(smtTimeout, smtAttempted)}% of attempted)");
    sb.AppendLine($"SMT failed/unknown: {smtFail} ({Percent(smtFail, smtAttempted)}% of attempted)");
    sb.AppendLine();

    sb.AppendLine("---- Verification 1000s ----");
    sb.AppendLine($"Verification attempted: {v1000Attempted}");
    sb.AppendLine($"Verification success: {v1000Pass} ({Percent(v1000Pass, v1000Attempted)}% of attempted)");
    sb.AppendLine($"Verification timeout: {v1000Timeout} ({Percent(v1000Timeout, v1000Attempted)}% of attempted)");
    sb.AppendLine($"Verification failed/unknown: {v1000Fail} ({Percent(v1000Fail, v1000Attempted)}% of attempted)");
    sb.AppendLine();

    sb.AppendLine("---- Verification 2500s ----");
    sb.AppendLine($"Verification attempted: {v2500Attempted}");
    sb.AppendLine($"Verification success: {v2500Pass} ({Percent(v2500Pass, v2500Attempted)}% of attempted)");
    sb.AppendLine($"Verification timeout: {v2500Timeout} ({Percent(v2500Timeout, v2500Attempted)}% of attempted)");
    sb.AppendLine($"Verification failed/unknown: {v2500Fail} ({Percent(v2500Fail, v2500Attempted)}% of attempted)");
    sb.AppendLine();

    sb.AppendLine("---- Verification 5000s ----");
    sb.AppendLine($"Verification attempted: {v5000Attempted}");
    sb.AppendLine($"Verification success: {v5000Pass} ({Percent(v5000Pass, v5000Attempted)}% of attempted)");
    sb.AppendLine($"Verification timeout: {v5000Timeout} ({Percent(v5000Timeout, v5000Attempted)}% of attempted)");
    sb.AppendLine($"Verification failed/unknown: {v5000Fail} ({Percent(v5000Fail, v5000Attempted)}% of attempted)");
    sb.AppendLine();
