...
class Program
{
            ...
            foreach (var watFile in files)
            {
                ...
                try
                {
                    ...
                    var translation = await RunProcess(
                        fileName: "dotnet",
                        arguments: $"{Quote(toolDll)} --wasm {Quote(watFile)} --no-verify",
                    ...
                    if (translationStatus is "PASS" or "PARTIAL")
                    {
                        ...
                        string smtFile = Path.GetFullPath(
                            Path.Combine(outDir, "smt", $"{logBase}.smt2")
                        );

                        Directory.CreateDirectory(Path.GetDirectoryName(smtFile)!);

                        if (File.Exists(smtFile))
                            File.Delete(smtFile);

                        var smt = await RunBoogie(
                            boogieExe,
                            $"/proc:BoogieEntry_* /traceTimes /proverLog:{smtFile} {Quote(boogieFile)}",
                            timeoutMs: SmtGenerationTimeoutSec * 1000
                        );

                        await File.WriteAllTextAsync(
                            Path.Combine(outDir, "logs", $"{logBase}_smt_command.txt"),
                            $"boogie command:\n{boogieExe} /proc:BoogieEntry_* /traceTimes /proverLog:{smtFile} {Quote(boogieFile)}\n\n"
                                + $"SMT expected at:\n{smtFile}\n\n"
                                + $"Exists after run: {File.Exists(smtFile)}\n"
                        );

                        if (File.Exists(smtFile) && new FileInfo(smtFile).Length > 0)
                        {
                            smtStatus = "PASS";
                        }
                        else if (smt.TimedOut)
                        {
                            smtStatus = ClassifyBoogie(smt, expectVerification: true);
                        }
                        else
                        {
                            smtStatus = "NO_SMT_FILE";
                        }
    ...
    static void WriteSummary(string path, List<ContractResult> results)
    {
        ...
        sb.AppendLine(
            $"Translation errors/failures: {errors} ({Percent(errors, total)}% of total)"
        );
        ...
        sb.AppendLine(
            $"SMT generated: {smtPass} ({Percent(smtPass, smtAttempted)}% of attempted, {Percent(smtPass, boogieGenerated)}% of Boogie files)"
        );
        sb.AppendLine(
            $"SMT timeout: {smtTimeout} ({Percent(smtTimeout, smtAttempted)}% of attempted)"
        );
        sb.AppendLine(
            $"SMT failed/unknown: {smtFail} ({Percent(smtFail, smtAttempted)}% of attempted)"
        );
        sb.AppendLine();
        ...
        sb.AppendLine(
            $"Verification success: {v1000Pass} ({Percent(v1000Pass, v1000Attempted)}% of attempted)"
        );
        sb.AppendLine(
            $"Verification timeout: {v1000Timeout} ({Percent(v1000Timeout, v1000Attempted)}% of attempted)"
        );
        sb.AppendLine(
            $"Verification failed/unknown: {v1000Fail} ({Percent(v1000Fail, v1000Attempted)}% of attempted)"
        );
        sb.AppendLine();
        ...
        sb.AppendLine(
            $"Verification success: {v2500Pass} ({Percent(v2500Pass, v2500Attempted)}% of attempted)"
        );
        sb.AppendLine(
            $"Verification timeout: {v2500Timeout} ({Percent(v2500Timeout, v2500Attempted)}% of attempted)"
        );
        sb.AppendLine(
            $"Verification failed/unknown: {v2500Fail} ({Percent(v2500Fail, v2500Attempted)}% of attempted)"
        );
        sb.AppendLine();
        ...
        sb.AppendLine(
            $"Verification success: {v5000Pass} ({Percent(v5000Pass, v5000Attempted)}% of attempted)"
        );
        sb.AppendLine(
            $"Verification timeout: {v5000Timeout} ({Percent(v5000Timeout, v5000Attempted)}% of attempted)"
        );
        sb.AppendLine(
            $"Verification failed/unknown: {v5000Fail} ({Percent(v5000Fail, v5000Attempted)}% of attempted)"
        );
        sb.AppendLine();
