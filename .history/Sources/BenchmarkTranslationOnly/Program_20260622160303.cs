...
class Program
{
    ...
    static readonly HashSet<string> Supported = new()
    {
        ...
        "i32.div_s", "i64.div_s", "i32.div_u", "i64.div_u", "f32.div", "f64.div",

        "i32.eq", "i64.eq", "f32.eq", "f64.eq",
        "i32.ne", "i64.ne", "f32.ne", "f64.ne",
        "i32.lt_s", "i64.lt_s", "i32.lt_u", "i64.lt_u", "f32.lt", "f64.lt",
        "i32.le_s", "i64.le_s", "i32.le_u", "i64.le_u", "f32.le", "f64.le",
        "i32.gt_s", "i64.gt_s", "i32.gt_u", "i64.gt_u", "f32.gt", "f64.gt",
        "i32.ge_s", "i64.ge_s", "i32.ge_u", "i64.ge_u", "f32.ge", "f64.ge",
        "i32.eqz", "i64.eqz",
        ...
        "i32.shl", "i64.shl", "i32.shr_s", "i64.shr_s", "i32.shr_u", "i64.shr_u",
        "i32.rotl", "i64.rotl", "i32.rotr", "i64.rotr",

        "i32.rem_s", "i64.rem_s", "i32.rem_u", "i64.rem_u",
        "i32.clz", "i64.clz", "i32.ctz", "i64.ctz", "i32.popcnt", "i64.popcnt",

        "f32.abs", "f64.abs", "f32.neg", "f64.neg", "f32.sqrt", "f64.sqrt",
        "f32.nearest", "f64.nearest", "f32.floor", "f64.floor",
        "f32.ceil", "f64.ceil", "f32.trunc", "f64.trunc",
        "f32.min", "f64.min", "f32.max", "f64.max", "f32.copysign", "f64.copysign",
    ...
    {
        "module", "type", "func", "param", "result", "local", "global", "mut",
        "then", "else", "memory", "data", "export", "import", "table", "elem",
        "tag", "rec", "field", "struct", "array", "offset", "align", "start",
    };
    ...
    static async Task<int> Main(string[] args)
    {
        ...
        foreach (var dataset in datasets)
        {
            ...
            var files = Directory.GetFiles(inputDir, "*.wat", SearchOption.AllDirectories)
                .OrderBy(x => x)
                .ToList();
            ...
            foreach (var watFile in files)
            {
                ...
                try
                {
                    ...
                    if (info.Length == 0)
                    {
                        var skipped = MakeSkippedResult(datasetName, watFile, "SKIP_EMPTY", "Empty WAT file.");
                    ...
                    double translationTimeMs = Math.Round(translationWatch.Elapsed.TotalMilliseconds, 2);
                    ...
                    if (!File.Exists(boogieFile))
                    {
                        var candidates = Directory.Exists(boogieOutDir)
                            ? Directory.GetFiles(boogieOutDir, "*.bpl", SearchOption.TopDirectoryOnly)
                ...
                {
                    var result = MakeSkippedResult(datasetName, watFile, "OUT_OF_MEMORY", ex.Message);
                    datasetResults.Add(result);
                    allResults.Add(result);
                    await File.WriteAllTextAsync(Path.Combine(outDir, "logs", $"{logBase}_oom.log"), ex.ToString());
                    PrintResultRow(result);
                    SaveProgress(outDir, datasetName, datasetResults, allResults);
                    GC.Collect();
                ...
                catch (Exception ex)
                {
                    ...
                    datasetResults.Add(result);
                    allResults.Add(result);
                    await File.WriteAllTextAsync(Path.Combine(outDir, "logs", $"{logBase}_error.log"), ex.ToString());
                    PrintResultRow(result);
                    SaveProgress(outDir, datasetName, datasetResults, allResults);
                }
            }

            WriteCsv(Path.Combine(outDir, $"{datasetName}_translation_only_results.csv"), datasetResults);
            WriteSummary(Path.Combine(outDir, $"{datasetName}_translation_only_summary.txt"), datasetResults);
            Console.WriteLine();
            Console.WriteLine(File.ReadAllText(Path.Combine(outDir, $"{datasetName}_translation_only_summary.txt")));
        }
        ...
        Console.WriteLine(File.ReadAllText(Path.Combine(outDir, "all_translation_only_summary.txt")));
        Console.WriteLine($"Total benchmark time: {globalWatch.Elapsed}");
        return 0;
    }

    static ContractResult MakeSkippedResult(string datasetName, string watFile, string status, string reason)
    {
    ...
    static void WriteSummary(string path, List<ContractResult> results)
    {
        ...
        int boogieGenerated = results.Count(r => r.TranslationStatus == "PASS" || r.TranslationStatus == "PARTIAL");
        ...
        int translationFail = results.Count(r => r.TranslationStatus == "FAIL" || r.TranslationStatus == "ERROR");
        ...
        double avgTranslationMs = results.Count == 0 ? 0 : Math.Round(results.Average(r => r.TranslationTimeMs), 2);
        var noVerifyTimes = results.Where(r => r.BoogieNoVerifyStatus != "NOT_RUN").Select(r => r.BoogieNoVerifyTimeMs).ToList();
        double avgNoVerifyMs = noVerifyTimes.Count == 0 ? 0 : Math.Round(noVerifyTimes.Average(), 2);
        ...
        sb.AppendLine($"  - PARTIAL translations: {translatedPartial} ({Percent(translatedPartial, total)}% of total)");
        sb.AppendLine($"Translation timeout: {translationTimeout} ({Percent(translationTimeout, total)}% of total)");
        sb.AppendLine($"Translation out-of-memory: {translationOom} ({Percent(translationOom, total)}% of total)");
        sb.AppendLine($"Translation errors/failures: {translationFail} ({Percent(translationFail, total)}% of total)");
        ...
        sb.AppendLine($"Boogie /noVerify timeout: {noVerifyTimeout} ({Percent(noVerifyTimeout, noVerifyAttempted)}% of attempted)");
        sb.AppendLine($"Boogie /noVerify out-of-memory: {noVerifyOom} ({Percent(noVerifyOom, noVerifyAttempted)}% of attempted)");
        sb.AppendLine($"Boogie /noVerify failed: {noVerifyFail} ({Percent(noVerifyFail, noVerifyAttempted)}% of attempted)");
        sb.AppendLine();
