...
static bool IsStackOrMemoryFailure(string output)
{
    ...
    return
        // .NET stack overflow
        output.Contains("Stack overflow", StringComparison.OrdinalIgnoreCase)

        // .NET heap out-of-memory
        || output.Contains("OutOfMemoryException", StringComparison.OrdinalIgnoreCase)
        ...
        || output.Contains("GC allocation failed", StringComparison.OrdinalIgnoreCase)

        // Linux OOM killer
        || output.Contains("Killed", StringComparison.OrdinalIgnoreCase)
        || output.Contains("exit code 137", StringComparison.OrdinalIgnoreCase);
}
if (!fullySupportedWat)
{
    var result = new ContractResult(
        ...
        TranslationStatus: "NOT_TRANSLATED_UNSUPPORTED",
        BoogieNoVerifyStatus: "NOT_RUN",
        TranslationTimeMs: 0,
        BoogieNoVerifyTimeMs: 0,
        UnsupportedOps: unsupportedOpsText,
        Reason: "WAT not fully supported; translation not attempted."
    );
...
else if (IsStackOrMemoryFailure(translation.Output))
{
    translationStatus = "OUT_OF_MEMORY";
...
int unsupportedNotTranslated = results.Count(r =>
    r.TranslationStatus == "NOT_TRANSLATED_UNSUPPORTED"
);
...
sb.AppendLine(
    $"Unsupported WAT not translated: {unsupportedNotTranslated} ({Percent(unsupportedNotTranslated, total)}% of total)"
);       
        sb.AppendLine(
