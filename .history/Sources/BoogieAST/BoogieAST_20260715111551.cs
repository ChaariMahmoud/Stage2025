...
";
    ...
    //DataType Support for BoogieAst

    public class BoogieDatatypeDecl : BoogieNamedDecl
    {
        public List<BoogieDatatypeConstructor> Constructors { get; }

        public BoogieDatatypeDecl(
            string name,
            List<BoogieDatatypeConstructor> constructors,
            List<BoogieAttribute> attributes = null
        )
        {
            Name = name;
            Constructors = constructors ?? new List<BoogieDatatypeConstructor>();
        ...
        public override string ToString()
        {
            ...
            builder.Append("datatype ").Append(Name).AppendLine(" {");

            for (int i = 0; i < Constructors.Count; i++)
            {
                builder.Append("    ").Append(Constructors[i]);
    ...
    public class BoogieDatatypeConstructor : BoogieASTNode
    {
        public string Name { get; }
        public List<BoogieDatatypeField> Fields { get; }

        public BoogieDatatypeConstructor(string name, List<BoogieDatatypeField> fields = null)
        {
            Name = name;
            Fields = fields ?? new List<BoogieDatatypeField>();
        }

        public class BoogieIsConstructorExpr : BoogieExpr
{
    public BoogieExpr Expr { get; }
    public string ConstructorName { get; }

    public BoogieIsConstructorExpr(BoogieExpr expr, string constructorName)
    {
        Expr = expr ?? throw new ArgumentNullException(nameof(expr));

        if (string.IsNullOrWhiteSpace(constructorName))
            throw new ArgumentException(
                "The constructor name cannot be empty.",
                nameof(constructorName)
            );
    ...
    public override string ToString()
    {
        return $"({Expr} is {ConstructorName})";
    }
}

public class BoogieDatatypeFieldAccessExpr : BoogieExpr
{
    public BoogieExpr Expr { get; }
    public string FieldName { get; }

    public BoogieDatatypeFieldAccessExpr(BoogieExpr expr, string fieldName)
    {
        Expr = expr ?? throw new ArgumentNullException(nameof(expr));

        if (string.IsNullOrWhiteSpace(fieldName))
            throw new ArgumentException(
                "The datatype field name cannot be empty.",
                nameof(fieldName)
            );
    ...
    public class BoogieDatatypeField : BoogieASTNode
    {
        public string Name { get; }
        public BoogieType Type { get; }

        public BoogieDatatypeField(string name, BoogieType type)
        {
            Name = name;
            Type = type;
        }
