...
namespace WasmToBoogie.Conversion
{
    public partial class WasmAstToBoogie
    {
        private void AddPreludeMath(BoogieProgram program, PreludeOptions opt)
        {
            ...
            AddCeilReal(program);
AddTruncReal(program);
AddCopysignReal(program);
        }

        private static void AddCeilReal(BoogieProgram program)
{
    ...
    program.Declarations.Add(new BoogieFunction("ceil_real", new() { x }, new() { r }));
}

private static void AddTruncReal(BoogieProgram program)
{
    var x = new BoogieFormalParam(new BoogieTypedIdent("x", BoogieType.Real));
    var r = new BoogieFormalParam(new BoogieTypedIdent("result", BoogieType.Real));

    program.Declarations.Add(new BoogieFunction("trunc_real", new() { x }, new() { r }));
}

private static void AddCopysignReal(BoogieProgram program)
{
    var x = new BoogieFormalParam(new BoogieTypedIdent("x", BoogieType.Real));
    var y = new BoogieFormalParam(new BoogieTypedIdent("y", BoogieType.Real));
    var r = new BoogieFormalParam(new BoogieTypedIdent("result", BoogieType.Real));

    program.Declarations.Add(new BoogieFunction("copysign_real", new() { x, y }, new() { r }));
}
