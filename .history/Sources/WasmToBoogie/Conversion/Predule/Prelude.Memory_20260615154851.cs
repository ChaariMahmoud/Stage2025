        ...
        private static void AddMemoryFillCopyProcedures(BoogieProgram program)
{
    ...
    var fillIns = new List<BoogieVariable>
    {
    ...
    var fillBody = new BoogieStmtList();

    // modèle abstrait : on havoc la mémoire pour représenter la modification
    fillBody.AddStatement(new BoogieHavocCmd(new BoogieIdentifierExpr("$mem")));

    program.Declarations.Add(
        new BoogieImplementation("memory_fill", fillIns, new(), new(), fillBody)
    );

    var copyIns = new List<BoogieVariable>
{
    new BoogieFormalParam(new BoogieTypedIdent("dst", BoogieType.Int)),
...
var copyBody = new BoogieStmtList();

// modèle abstrait : memory.copy modifie la mémoire
copyBody.AddStatement(new BoogieHavocCmd(new BoogieIdentifierExpr("$mem")));

program.Declarations.Add(
    new BoogieImplementation("memory_copy", copyIns, new(), new(), copyBody)
);
}
        ...
        private static void AddMemorySizeGrowProcedures(BoogieProgram program)
{
    // memory.size() -> result:int
    {
        ...
        program.Declarations.Add(
            new BoogieProcedure("memory_size", new(), outs, new() { new BoogieAttribute("inline", 1) }, new(), new(), new())
        );

        var body = new BoogieStmtList();
        body.AddStatement(new BoogieAssignCmd(new BoogieIdentifierExpr("result"), new BoogieIdentifierExpr("$mem_pages")));

        program.Declarations.Add(new BoogieImplementation("memory_size", new(), outs, new(), body));
    }
    ...
    // memory.grow(delta:int) -> oldSize:int
    {
        ...
        program.Declarations.Add(
            new BoogieProcedure("memory_grow", ins, outs, new() { new BoogieAttribute("inline", 1) }, mods, new(), new())
        );

        var body = new BoogieStmtList();
        body.AddStatement(new BoogieAssignCmd(new BoogieIdentifierExpr("oldSize"), new BoogieIdentifierExpr("$mem_pages")));
        body.AddStatement(
            new BoogieAssignCmd(
        ...
        program.Declarations.Add(new BoogieImplementation("memory_grow", ins, outs, new(), body));
    }
}
