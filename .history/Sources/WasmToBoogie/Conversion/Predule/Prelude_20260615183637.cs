namespace WasmToBoogie.Conversion
{
    public partial class WasmAstToBoogie
    {
        ...
        private void AddPrelude(BoogieAST.BoogieProgram program)
        {
            ...
            if (opt.Sections.HasFlag(PreludeSection.Bitwise))
                AddPreludeBitwise(program);

            if (opt.Sections.HasFlag(PreludeSection.IntOps))
                AddPreludeIntOps(program);
            if (opt.Sections.HasFlag(PreludeSection.IntUnaryOps))
                AddPreludeIntUnaryOps(program);

            if (opt.Sections.HasFlag(PreludeSection.StackOps))
                AddPreludeStackOps(program);
            if (opt.Sections.HasFlag(PreludeSection.Table))
                AddPreludeTable(program);
        }
    }
}
