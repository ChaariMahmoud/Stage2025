        ...
        private void TranslateNode(WasmNode node, BoogieStmtList body)
        {
            switch (node)
            {
                ...
                case UnaryOpNode un:
                {
                    ...
                    else if (un.Op is "i32.clz" or "i64.clz" or "i32.ctz" or "i64.ctz" or "i32.popcnt" or "i64.popcnt")
{
    body.AddStatement(new BoogieCallCmd("popToTmp1", new(), new()));
    ...
    string fun = un.Op switch
    {
        ...
        _ => throw new NotSupportedException($"Unsupported integer unary op: {un.Op}")
    };

    body.AddStatement(
        new BoogieCallCmd(
            "push",
            new() { new BoogieFunctionCall(fun, new() { new BoogieIdentifierExpr("$tmp1") }) },
            new()
        )
    );
}
