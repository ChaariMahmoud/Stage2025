...
private static BoogieExpr I32Value(BoogieExpr e) =>
    Field(e, "value_i32");

private static BoogieExpr I64Value(BoogieExpr e) =>
    Field(e, "value_i64");

private static BoogieExpr F32Value(BoogieExpr e) =>
    Field(e, "value_f32");

private static BoogieExpr F64Value(BoogieExpr e) =>
    Field(e, "value_f64");
    private void EmitTypedBinaryArithmetic(string op, BoogieStmtList body)
{
    ...
    body.AddStatement(
        new BoogieAssertCmd(IsCtor(Tmp1(), ctor))
    );

    body.AddStatement(
        new BoogieAssertCmd(IsCtor(Tmp2(), ctor))
    );

    var lhs = Field(Tmp2(), field);
    var rhs = Field(Tmp1(), field);

    var rawResult = new BoogieBinaryOperation(
        bop,
        lhs,
        rhs
    );
                    ...
                    case BinaryOpNode bn:
{
    ...
    if (bn.Op is
        ...
        "f32.add" or "f32.sub" or "f32.mul" or
        "f64.add" or "f64.sub" or "f64.mul")
    {
    ...
    break;
                    ...
                    case SelectNode sel:private static BoogieExpr Tmp1() => Id("$tmp1");
private static BoogieExpr Tmp2() => Id("$tmp2");
