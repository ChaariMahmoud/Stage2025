        ...
        private void EmitTypedComparison(string op, BoogieStmtList body)
        {
            ...
            switch (op)
            {
                ...
                case "i32.eq":
                    ctor = "I32";
                    field = "value_i32";
                    cmp = BoogieBinaryOperation.Opcode.EQ;
                    break;

                case "i32.ne":
                    ctor = "I32";
                    field = "value_i32";
                    cmp = BoogieBinaryOperation.Opcode.NEQ;
                    break;
                ...
                case "i32.lt_u":
                    ctor = "I32";
                    field = "value_i32";
                    cmp = BoogieBinaryOperation.Opcode.LT;
                    break;
                ...
                case "i32.gt_u":
                    ctor = "I32";
                    field = "value_i32";
                    cmp = BoogieBinaryOperation.Opcode.GT;
                    break;
                ...
                case "i32.le_u":
                    ctor = "I32";
                    field = "value_i32";
                    cmp = BoogieBinaryOperation.Opcode.LE;
                    break;
                ...
                case "i32.ge_u":
                    ctor = "I32";
                    field = "value_i32";
                    cmp = BoogieBinaryOperation.Opcode.GE;
                    break;
            ...
            var cond = new BoogieBinaryOperation(cmp, lhs, rhs);
        ...
        private void EmitTypedEqz(string op, BoogieStmtList body)
{
    ...
    body.AddStatement(
        new BoogieCallCmd(
            "push",
            new List<BoogieExpr> { BoolToI32(cond) },
            new()
        )
    );
}
...
private void EmitTypedFloatUnary(string op, BoogieStmtList body)
{
    ...
    switch (op)
    {
        ...
        case "f32.neg":
        case "f64.neg":
            rawResult = new BoogieUnaryOperation(
        ...
        case "f32.floor":
        case "f64.floor":
            rawResult = new BoogieFunctionCall("floor_real", new List<BoogieExpr> { value });
            break;

        case "f32.nearest":
        case "f64.nearest":
            rawResult = new BoogieFunctionCall("nearest_real", new List<BoogieExpr> { value });
            break;
    ...
    var typedResult = new BoogieFuncCallExpr(
        ctor,
        new List<BoogieExpr> { rawResult }
    );

    body.AddStatement(
        new BoogieCallCmd(
            "push",
            new List<BoogieExpr> { typedResult },
            new()
        )
    );
}

private static bool IsTypedEqzOp(string op) =>
    op is "i32.eqz" or "i64.eqz";

private static bool IsTypedFloatUnaryOp(string op) =>
    op is
        "f32.abs" or "f64.abs" or
        "f32.neg" or "f64.neg" or
        "f32.sqrt" or "f64.sqrt" or
        "f32.floor" or "f64.floor" or
        "f32.nearest" or "f64.nearest";
        ...
        private void EmitTypedBinaryArithmetic(string op, BoogieStmtList body)
        {
            ...
            var typedResult = new BoogieFuncCallExpr(ctor, new List<BoogieExpr> { rawResult });
        ...
        private (BoogieProcedure, BoogieImplementation) TranslateFunction(WasmFunction func)
        {
            ...
            // Locals init 0
            for (int i = n; i < n + m; i++)
                body.AddStatement(new BoogieAssignCmd(indexToId[i], UndefValue()));
...
case ConstNode cn:
{
    ...
    if (ty == "i32")
    {
        if (System.Numerics.BigInteger.TryParse(cn.Value, out var v))
        {
            var lit = new BoogieLiteralExpr(v);

            body.AddStatement(
                new BoogieCallCmd(
                    "push",
                    new List<BoogieExpr> { I32(ToU32(lit)) },
                    new()
                )
            );
        }
    ...
    else if (ty == "i64")
    {
        if (System.Numerics.BigInteger.TryParse(cn.Value, out var v))
        {
            ...
            body.AddStatement(
                new BoogieCallCmd(
                    "push",
                    new List<BoogieExpr> { I64(ToU64(lit)) },
                    new()
                )
            );
        }
                    ...
                    case UnaryOpNode un:
{
    ...
    else if (un.Op == "i32.wrap_i64")
    {
        ...
        var value = Field(Tmp1(), "value_i64");

        body.AddStatement(
            new BoogieCallCmd(
                "push",
                new List<BoogieExpr> { I32(value) },
                new()
            )
        );
    }
    ...
    {
        body.AddStatement(
            new BoogieCommentCmd($"// unsupported unary op in typed stage: {un.Op}")
        );
    }
