...
private List<BoogieExpr> GetGlobalInvariantExprs()
{
    ...
    SpecToBoogieTranslator translator =
        GetSpecTranslator();

    foreach (var invariant in moduleSpec.GlobalInvariants)
    {
        invariants.Add(
            translator.Translate(invariant)
        );
    }
...
if (moduleSpec != null && func.Name != null)
{
    ...
    if (
        moduleSpec.RequiresByFunc.TryGetValue(
            func.Name,
            out var requires
        )
    )
    {
        ...
        {
            pre.Add(
                translator.Translate(requirement)
            );
        }
    }

    if (
        moduleSpec.EnsuresByFunc.TryGetValue(
            func.Name,
            out var ensures
        )
    )
