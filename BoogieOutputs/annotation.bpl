...
procedure {:inline 1} CorralChoice_annotation();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_annotation()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_annotation();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_annotation()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_annotation();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_annotation()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_annotation();
    }
}
