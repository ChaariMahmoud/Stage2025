...
procedure {:inline 1} CorralChoice_bitwise();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
implementation CorralChoice_bitwise()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_bitwise();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
implementation BoogieEntry_bitwise()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_bitwise();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
implementation CorralEntry_bitwise()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_bitwise();
    }
}
