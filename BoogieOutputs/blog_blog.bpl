...
axiom((global_1) == (17522.0));
const global_2: real;

axiom((global_2) == (17522.0));
...
label$8_start_162:
    ...
    call popToTmp1();
    if (real_to_bool($tmp1)) {
        goto label$12_end_168;
    }
    goto label$13_end_169;
label$14_end_170:
    call push(loc3);
    call popToTmp1();
    idx := (real_to_int($tmp1)) + (28);
...
procedure {:inline 1} CorralChoice_blog_blog();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies global_0;
implementation CorralChoice_blog_blog()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_blog_blog();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies global_0;
implementation BoogieEntry_blog_blog()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_blog_blog();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies global_0;
implementation CorralEntry_blog_blog()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_blog_blog();
    }
}
