...
axiom((global_1) == (17576.0));
const global_2: real;

axiom((global_2) == (17576.0));
...
procedure {:inline 1} CorralChoice_cms_cms();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies global_0;
implementation CorralChoice_cms_cms()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
modifies $mem;
modifies global_0;
implementation BoogieEntry_cms_cms()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
modifies $mem;
modifies global_0;
implementation CorralEntry_cms_cms()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_cms_cms();
    }
}
