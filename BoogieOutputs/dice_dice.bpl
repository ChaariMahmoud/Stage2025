...
axiom((global_1) == (17038.0));
const global_2: real;

axiom((global_2) == (17038.0));
...
label$17_end_54:
    ...
    call popToTmp1();
    if (real_to_bool($tmp1)) {
        goto label$14_end_51;
    }
    goto label$13_end_50;
label$16_end_53:
    call push(loc2);
    call push(-48.0);
    call popToTmp1();
...
procedure {:inline 1} CorralChoice_dice_dice();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies global_0;
implementation CorralChoice_dice_dice()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_dice_dice();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies global_0;
implementation BoogieEntry_dice_dice()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_dice_dice();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies global_0;
implementation CorralEntry_dice_dice()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_dice_dice();
    }
}
