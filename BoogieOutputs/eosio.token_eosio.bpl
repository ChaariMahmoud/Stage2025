...
label$21_end_489:
    ...
    call popToTmp1();
    if (real_to_bool($tmp1)) {
        goto label$18_end_486;
    }
    goto label$17_end_485;
label$20_end_488:
    call push(loc1);
    call push(208.0);
    call popToTmp1();
...
procedure {:inline 1} CorralChoice_eosio_token_eosio_token();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
modifies $mem;
implementation CorralChoice_eosio_token_eosio_token()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_eosio_token_eosio_token();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
modifies $mem;
implementation BoogieEntry_eosio_token_eosio_token()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_eosio_token_eosio_token();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
modifies $mem;
implementation CorralEntry_eosio_token_eosio_token()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_eosio_token_eosio_token();
    }
}
