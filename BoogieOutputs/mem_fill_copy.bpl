...
implementation func_0()
{
    ...
    call popToTmp2();
    call popToTmp3();
    call memory_fill(real_to_int($tmp3), real_to_int($tmp2), real_to_int($tmp1));
    call push(20.0);
    call push(0.0);
    call push(10.0);
    call popToTmp1();
    call popToTmp2();
    call popToTmp3();
    call memory_copy(real_to_int($tmp3), real_to_int($tmp2), real_to_int($tmp1));
}

procedure {:inline 1} CorralChoice_mem_fill_copy();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_mem_fill_copy()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_mem_fill_copy();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_mem_fill_copy()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_mem_fill_copy();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_mem_fill_copy()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_mem_fill_copy();
    }
}
