...
procedure {:inline 1} CorralChoice_mem_size_grow();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_mem_size_grow()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_mem_size_grow();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_mem_size_grow()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_mem_size_grow();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_mem_size_grow()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_mem_size_grow();
    }
}
