...
axiom((global_1) == (16798.0));
const global_2: real;

axiom((global_2) == (16798.0));
...
procedure {:inline 1} CorralChoice_merkle_merkle();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies global_0;
implementation CorralChoice_merkle_merkle()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_merkle_merkle();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies global_0;
implementation BoogieEntry_merkle_merkle()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_merkle_merkle();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies global_0;
implementation CorralEntry_merkle_merkle()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_merkle_merkle();
    }
}
