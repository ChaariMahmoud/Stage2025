...
procedure {:inline 1} CorralChoice_multi_mem();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_multi_mem()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_multi_mem();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_multi_mem()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_multi_mem();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_multi_mem()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_multi_mem();
    }
}
