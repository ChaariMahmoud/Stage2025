...
procedure {:inline 1} CorralChoice_num_test();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
implementation CorralChoice_num_test()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_num_test();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
implementation BoogieEntry_num_test()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_num_test();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
implementation CorralEntry_num_test()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_num_test();
    }
}
