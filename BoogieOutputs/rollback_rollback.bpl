...
label$23_end_64:
    ...
    call popToTmp1();
    if (real_to_bool($tmp1)) {
        goto label$20_end_61;
    }
    goto label$19_end_60;
label$22_end_63:
    call push(loc1);
    call push(208.0);
    call popToTmp1();
...
procedure {:inline 1} CorralChoice_rollback_rollback();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
modifies $mem;
implementation CorralChoice_rollback_rollback()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_rollback_rollback();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
modifies $mem;
implementation BoogieEntry_rollback_rollback()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_rollback_rollback();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
modifies $stack;
modifies $mem;
implementation CorralEntry_rollback_rollback()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_rollback_rollback();
    }
}
