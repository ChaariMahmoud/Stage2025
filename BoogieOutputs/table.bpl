...
implementation func_0()
{
    ...
    entry_sp := $sp;
    call idx := table_size();
    call push(int_to_real(idx));
    call pop();
    call push(0.0);
    // // unhandled raw instruction: ref.null
    call popToTmp1();
    call popToTmp2();
    ...
    call popToTmp2();
    idx := real_to_int($tmp1);
    call load_i := table_grow($tmp2, idx);
...
procedure {:inline 1} CorralChoice_table();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_table()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_table()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
