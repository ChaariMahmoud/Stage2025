...
procedure {:inline 1} CorralChoice_test_conditional();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_test_conditional()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_test_conditional();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_test_conditional()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_test_conditional();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_test_conditional()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_test_conditional();
    }
}
