...
procedure {:inline 1} CorralChoice_br_if();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_br_if()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_br_if()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_br_if()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_br_if();
    }
}
