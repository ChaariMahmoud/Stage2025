...
procedure {:inline 1} CorralChoice_br_table();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_br_table()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_br_table();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_br_table()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_br_table();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_br_table()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_br_table();
    }
}
