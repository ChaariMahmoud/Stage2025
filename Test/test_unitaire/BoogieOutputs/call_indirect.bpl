...
procedure {:inline 1} call_indirect();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
procedure {:inline 1} CorralChoice_call_indirect();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_call_indirect()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (2)));
...
procedure BoogieEntry_call_indirect();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_call_indirect()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_call_indirect();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_call_indirect()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_call_indirect();
    }
}
