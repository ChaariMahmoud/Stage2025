...
procedure {:inline 1} CorralChoice_eq();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_eq()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_eq()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_eq();
    }
}
