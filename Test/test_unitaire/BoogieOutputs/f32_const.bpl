...
procedure {:inline 1} CorralChoice_f32_const();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_f32_const()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_f32_const();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_f32_const()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_f32_const();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_f32_const()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_f32_const();
    }
}
