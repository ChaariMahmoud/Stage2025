...
const my_global: real;

axiom((my_global) == (42.0));
...
procedure {:inline 1} CorralChoice_global_get();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_global_get()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_global_get();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_global_get()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_global_get();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_global_get()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_global_get();
    }
}
