...
implementation global_set()
{
    ...
    entry_sp := $sp;
    call push(100.0);
    call my_global := popArgs1();
}

procedure {:inline 1} CorralChoice_global_set();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies my_global;
implementation CorralChoice_global_set()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_global_set();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies my_global;
implementation BoogieEntry_global_set()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_global_set();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem_pages;
modifies my_global;
implementation CorralEntry_global_set()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_global_set();
    }
}
