...
procedure {:inline 1} if_no_else_fonc();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation if_no_else_fonc()
{
    var arg1: real;
    var arg2: real;
    var entry_sp: int;
    ...
    if (real_to_bool($tmp1)) {
        // // unhandled raw instruction: then
    }
    call push(arg2);
}
...
procedure {:inline 1} CorralChoice_if_no_else_fonc();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_if_no_else_fonc()
{
    var c: int;
    var argTmp: real;
    havoc c;
    ...
    if ((c) == (0)) {
        ...
        assume ((argTmp) == (real(int(argTmp))));
        call push(argTmp);
        call if_no_else_fonc();
        call popDiscard1();
    }
}

procedure BoogieEntry_if_no_else_fonc();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_if_no_else_fonc()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_if_no_else_fonc();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_if_no_else_fonc()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_if_no_else_fonc();
    }
}
