...
procedure {:inline 1} if_void_else();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
procedure {:inline 1} CorralChoice_if_void_else();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_if_void_else()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_if_void_else();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_if_void_else()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_if_void_else();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_if_void_else()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_if_void_else();
    }
}
