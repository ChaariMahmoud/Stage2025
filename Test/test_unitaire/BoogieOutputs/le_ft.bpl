...
procedure {:inline 1} CorralChoice_le_ft();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_le_ft()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_le_ft()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_le_ft()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_le_ft();
    }
}
