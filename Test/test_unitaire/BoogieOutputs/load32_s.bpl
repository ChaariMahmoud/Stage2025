...
procedure {:inline 1} CorralChoice_load32_s();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_load32_s()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_load32_s();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_load32_s()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_load32_s();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_load32_s()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_load32_s();
    }
}
