...
procedure {:inline 1} CorralChoice_load_ft();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_load_ft()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_load_ft()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_load_ft()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_load_ft();
    }
}
