...
procedure {:inline 1} CorralChoice_local_set();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_local_set()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_local_set();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_local_set()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_local_set();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_local_set()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_local_set();
    }
}
