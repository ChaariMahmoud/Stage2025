...
procedure {:inline 1} CorralChoice_loop();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_loop()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_loop()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_loop()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_loop();
    }
}
