...
procedure {:inline 1} CorralChoice_lt_u();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_lt_u()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_lt_u()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_lt_u()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_lt_u();
    }
}
