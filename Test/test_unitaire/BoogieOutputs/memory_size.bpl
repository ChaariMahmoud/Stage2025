...
procedure {:inline 1} memory_size();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
procedure {:inline 1} CorralChoice_memory_size();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_memory_size()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_memory_size();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_memory_size()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_memory_size();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_memory_size()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_memory_size();
    }
}
