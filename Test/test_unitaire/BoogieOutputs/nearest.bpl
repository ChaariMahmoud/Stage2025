...
procedure {:inline 1} CorralChoice_nearest();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_nearest()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_nearest()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_nearest()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_nearest();
    }
}
