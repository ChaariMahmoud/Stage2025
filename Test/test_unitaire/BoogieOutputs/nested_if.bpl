...
implementation nested_if()
{
    ...
    if (real_to_bool($tmp1)) {
        ...
        // // unhandled raw instruction: else
        call popToTmp2();
        $tmp3 := $tmp2;
    }
    call push($tmp3);
...
procedure {:inline 1} CorralChoice_nested_if();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_nested_if()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_nested_if();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_nested_if()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_nested_if();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_nested_if()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_nested_if();
    }
}
