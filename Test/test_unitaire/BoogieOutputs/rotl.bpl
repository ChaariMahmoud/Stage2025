...
procedure {:inline 1} CorralChoice_rotl();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_rotl()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_rotl()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_rotl()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_rotl();
    }
}
