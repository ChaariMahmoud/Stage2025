...
procedure {:inline 1} CorralChoice_store_ft();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_store_ft()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_store_ft();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_store_ft()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_store_ft();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_store_ft()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_store_ft();
    }
}
