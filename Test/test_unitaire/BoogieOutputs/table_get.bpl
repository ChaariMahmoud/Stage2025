...
implementation table_get()
{
    ...
    call $tmp1 := table_get(idx);
    call push($tmp1);
    call pop();
}

procedure {:inline 1} CorralChoice_table_get();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_table_get()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_table_get();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_table_get()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_table_get();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_table_get()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_table_get();
    }
}
