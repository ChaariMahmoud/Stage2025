...
procedure {:inline 1} unreachable();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
procedure {:inline 1} CorralChoice_unreachable();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_unreachable()
{
    var c: int;
    havoc c;
    assume (((0) <= (c)) && ((c) < (1)));
...
procedure BoogieEntry_unreachable();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_unreachable()
{
    var c: int;
    call initGlobals();
    call InitRuntime();
...
procedure CorralEntry_unreachable();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_unreachable()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_unreachable();
    }
}
