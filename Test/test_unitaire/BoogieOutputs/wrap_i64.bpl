...
procedure {:inline 1} CorralChoice_wrap_i64();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralChoice_wrap_i64()
{
    var c: int;
    var argTmp: real;
    havoc c;
...
procedure BoogieEntry_wrap_i64();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation BoogieEntry_wrap_i64()
{
    var c: int;
    var argTmp: real;
    call initGlobals();
...
procedure CorralEntry_wrap_i64();
modifies $tmp1;
modifies $tmp2;
modifies $tmp3;
modifies $sp;
...
modifies $mem;
modifies $mem_pages;
implementation CorralEntry_wrap_i64()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_wrap_i64();
    }
}
