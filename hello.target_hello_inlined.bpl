// Boogie program verifier version 3.5.5.0, Copyright (c) 2003-2014, Microsoft.
// Command Line Options: /trace /tracePOs /errorTrace:2 /proc:BoogieEntry_hello_target_hello_target /print:hello.target_hello_inlined.bpl /trace /inline:spec /traceTimes /tracePOs BoogieOutputs/hello.target_hello.bpl

var $stack: [int]real;

var $sp: int;

var $tmp1: real;

var $tmp2: real;

var $tmp3: real;

var $mem: [int]int;

var $mem_pages: int;

function to_u8(x: int) : int;

axiom (forall x: int :: 0 <= to_u8(x) && to_u8(x) < 256);

procedure {:inline 1} mem_read_u8(a: int) returns (result: int);



implementation mem_read_u8(a: int) returns (result: int)
{
    result := $mem[a];
    assume 0 <= result && result < 256;
}



procedure {:inline 1} mem_read_s8(a: int) returns (result: int);



implementation mem_read_s8(a: int) returns (result: int)
{
  var u: int;

    call u := mem_read_u8(a);
    if (u >= 128)
    {
        result := u - 256;
    }
    else
    {
        result := u;
    }
}



procedure {:inline 1} mem_read_u16(a: int) returns (result: int);



implementation mem_read_u16(a: int) returns (result: int)
{
  var b0: int;
  var b1: int;

    call b0 := mem_read_u8(a);
    call b1 := mem_read_u8(a + 1);
    result := b0 + 256 * b1;
    assume 0 <= result && result < 65536;
}



procedure {:inline 1} mem_read_s16(a: int) returns (result: int);



implementation mem_read_s16(a: int) returns (result: int)
{
  var u: int;

    call u := mem_read_u16(a);
    if (u >= 32768)
    {
        result := u - 65536;
    }
    else
    {
        result := u;
    }
}



procedure {:inline 1} mem_read_u32(a: int) returns (result: int);



implementation mem_read_u32(a: int) returns (result: int)
{
  var b0: int;
  var b1: int;
  var b2: int;
  var b3: int;

    call b0 := mem_read_u8(a);
    call b1 := mem_read_u8(a + 1);
    call b2 := mem_read_u8(a + 2);
    call b3 := mem_read_u8(a + 3);
    result := b0 + 256 * b1 + 65536 * b2 + 16777216 * b3;
    assume 0 <= result && result < 4294967296;
}



procedure {:inline 1} mem_read_s32(a: int) returns (result: int);



implementation mem_read_s32(a: int) returns (result: int)
{
  var u: int;

    call u := mem_read_u32(a);
    if (u >= 2147483648)
    {
        result := u - 4294967296;
    }
    else
    {
        result := u;
    }
}



procedure {:inline 1} mem_read_u64(a: int) returns (result: int);



implementation mem_read_u64(a: int) returns (result: int)
{
  var b0: int;
  var b1: int;
  var b2: int;
  var b3: int;
  var b4: int;
  var b5: int;
  var b6: int;
  var b7: int;

    call b0 := mem_read_u8(a);
    call b1 := mem_read_u8(a + 1);
    call b2 := mem_read_u8(a + 2);
    call b3 := mem_read_u8(a + 3);
    call b4 := mem_read_u8(a + 4);
    call b5 := mem_read_u8(a + 5);
    call b6 := mem_read_u8(a + 6);
    call b7 := mem_read_u8(a + 7);
    result := b0
       + 
      256 * b1
       + 
      65536 * b2
       + 
      16777216 * b3
       + 
      4294967296 * b4
       + 
      1099511627776 * b5
       + 
      281474976710656 * b6
       + 72057594037927936 * b7;
}



procedure {:inline 1} mem_read_s64(a: int) returns (result: int);



implementation mem_read_s64(a: int) returns (result: int)
{
  var u: int;

    call u := mem_read_u64(a);
    result := u;
}



function byte0_16(x: int) : int;

axiom (forall x: int :: 0 <= byte0_16(x) && byte0_16(x) < 256);

function byte1_16(x: int) : int;

axiom (forall x: int :: 0 <= byte1_16(x) && byte1_16(x) < 256);

function byte0_32(x: int) : int;

axiom (forall x: int :: 0 <= byte0_32(x) && byte0_32(x) < 256);

function byte1_32(x: int) : int;

axiom (forall x: int :: 0 <= byte1_32(x) && byte1_32(x) < 256);

function byte2_32(x: int) : int;

axiom (forall x: int :: 0 <= byte2_32(x) && byte2_32(x) < 256);

function byte3_32(x: int) : int;

axiom (forall x: int :: 0 <= byte3_32(x) && byte3_32(x) < 256);

function byte0_64(x: int) : int;

axiom (forall x: int :: 0 <= byte0_64(x) && byte0_64(x) < 256);

function byte1_64(x: int) : int;

axiom (forall x: int :: 0 <= byte1_64(x) && byte1_64(x) < 256);

function byte2_64(x: int) : int;

axiom (forall x: int :: 0 <= byte2_64(x) && byte2_64(x) < 256);

function byte3_64(x: int) : int;

axiom (forall x: int :: 0 <= byte3_64(x) && byte3_64(x) < 256);

function byte4_64(x: int) : int;

axiom (forall x: int :: 0 <= byte4_64(x) && byte4_64(x) < 256);

function byte5_64(x: int) : int;

axiom (forall x: int :: 0 <= byte5_64(x) && byte5_64(x) < 256);

function byte6_64(x: int) : int;

axiom (forall x: int :: 0 <= byte6_64(x) && byte6_64(x) < 256);

function byte7_64(x: int) : int;

axiom (forall x: int :: 0 <= byte7_64(x) && byte7_64(x) < 256);

procedure {:inline 1} mem_write_u8(a: int, v: int);
  modifies $mem;



implementation mem_write_u8(a: int, v: int)
{
    $mem[a] := to_u8(v);
}



procedure {:inline 1} mem_write_u16(a: int, v: int);
  modifies $mem;



implementation mem_write_u16(a: int, v: int)
{
    call mem_write_u8(a, byte0_16(v));
    call mem_write_u8(a + 1, byte1_16(v));
}



procedure {:inline 1} mem_write_u32(a: int, v: int);
  modifies $mem;



implementation mem_write_u32(a: int, v: int)
{
    call mem_write_u8(a, byte0_32(v));
    call mem_write_u8(a + 1, byte1_32(v));
    call mem_write_u8(a + 2, byte2_32(v));
    call mem_write_u8(a + 3, byte3_32(v));
}



procedure {:inline 1} mem_write_u64(a: int, v: int);
  modifies $mem;



implementation mem_write_u64(a: int, v: int)
{
    call mem_write_u8(a, byte0_64(v));
    call mem_write_u8(a + 1, byte1_64(v));
    call mem_write_u8(a + 2, byte2_64(v));
    call mem_write_u8(a + 3, byte3_64(v));
    call mem_write_u8(a + 4, byte4_64(v));
    call mem_write_u8(a + 5, byte5_64(v));
    call mem_write_u8(a + 6, byte6_64(v));
    call mem_write_u8(a + 7, byte7_64(v));
}



function nd_real() : real;

function bool_to_real(b: bool) : real
uses {
axiom (forall b: bool :: 
  { bool_to_real(b): real } 
  bool_to_real(b): real == (if b then 1e0 else 0e0));
}

function real_to_bool(r: real) : bool
uses {
axiom (forall r: real :: 
  { real_to_bool(r): bool } 
  real_to_bool(r): bool == (if r == 0e0 then false else true));
}

axiom (forall b: bool :: 
  { bool_to_real(b) } 
  bool_to_real(b) == 0e0 || bool_to_real(b) == 1e0);

axiom (forall b: bool :: 
  { real_to_bool(bool_to_real(b)) } 
  real_to_bool(bool_to_real(b)) == b);

axiom (forall r: real :: { real_to_bool(r) } real_to_bool(r) == false <==> r == 0e0);

function real_to_int(r: real) : int;

function int_to_real(i: int) : real;

function bits32_to_real(i: int) : real;

function bits64_to_real(i: int) : real;

procedure {:inline 1} InitRuntime();
  modifies $sp, $tmp1, $tmp2, $tmp3;
  ensures $sp == 0;
  ensures 0 <= $sp;



implementation InitRuntime()
{
    $sp := 0;
    $tmp1 := 0e0;
    $tmp2 := 0e0;
    $tmp3 := 0e0;
}



procedure {:inline 1} push(val: real);
  requires 0 <= $sp;
  modifies $sp, $stack;
  ensures $sp == old($sp) + 1;
  ensures $stack[old($sp)] == val;
  ensures (forall i: int :: i != old($sp) ==> $stack[i] == old($stack)[i]);
  ensures 0 <= $sp;



implementation push(val: real)
{
    $stack[$sp] := val;
    $sp := $sp + 1;
}



procedure {:inline 1} popToTmp1();
  requires $sp > 0;
  modifies $sp, $tmp1;
  ensures $sp == old($sp) - 1;
  ensures (forall i: int :: $stack[i] == old($stack)[i]);
  ensures $tmp1 == old($stack)[old($sp) - 1];
  ensures 0 <= $sp;



implementation popToTmp1()
{
    $sp := $sp - 1;
    $tmp1 := $stack[$sp];
}



procedure {:inline 1} popToTmp2();
  requires $sp > 0;
  modifies $sp, $tmp2;
  ensures $sp == old($sp) - 1;
  ensures (forall i: int :: $stack[i] == old($stack)[i]);
  ensures $tmp2 == old($stack)[old($sp) - 1];
  ensures 0 <= $sp;



implementation popToTmp2()
{
    $sp := $sp - 1;
    $tmp2 := $stack[$sp];
}



procedure {:inline 1} popToTmp3();
  requires $sp > 0;
  modifies $sp, $tmp3;
  ensures $sp == old($sp) - 1;
  ensures (forall i: int :: $stack[i] == old($stack)[i]);
  ensures $tmp3 == old($stack)[old($sp) - 1];
  ensures 0 <= $sp;



implementation popToTmp3()
{
    $sp := $sp - 1;
    $tmp3 := $stack[$sp];
}



procedure {:inline 1} pop();
  requires $sp > 0;
  modifies $sp;
  ensures $sp == old($sp) - 1;
  ensures 0 <= $sp;



implementation pop()
{
    $sp := $sp - 1;
}



procedure {:inline 1} initGlobals();



implementation initGlobals()
{
}



procedure {:inline 1} action_data_size();
  modifies $tmp1, $sp, $stack;



implementation action_data_size()
{
}



procedure {:inline 1} current_time();
  modifies $tmp1, $sp, $stack;



implementation current_time()
{
}



procedure {:inline 1} eosio_assert();
  modifies $tmp1, $sp, $stack;



implementation eosio_assert()
{
}



procedure {:inline 1} memcpy();
  modifies $tmp1, $sp, $stack;



implementation memcpy()
{
}



procedure {:inline 1} printn();
  modifies $tmp1, $sp, $stack;



implementation printn()
{
}



procedure {:inline 1} prints();
  modifies $tmp1, $sp, $stack;



implementation prints()
{
}



procedure {:inline 1} read_action_data();
  modifies $tmp1, $sp, $stack;



implementation read_action_data()
{
}



procedure {:inline 1} require_auth();
  modifies $tmp1, $sp, $stack;



implementation require_auth()
{
}



procedure {:inline 1} require_auth2();
  modifies $tmp1, $sp, $stack;



implementation require_auth2()
{
}



procedure {:inline 1} fimport_0();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_0()
{
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
}



procedure {:inline 1} fimport_1();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_1()
{
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
}



procedure {:inline 1} popArgs2() returns (a1: real, a2: real);
  requires $sp >= 2;
  modifies $sp, $stack;
  ensures $sp == old($sp) - 2;
  ensures 0 <= $sp;
  ensures (forall i: int :: $stack[i] == old($stack)[i]);
  ensures a1 == old($stack)[old($sp) - 2];
  ensures a2 == old($stack)[old($sp) - 1];



implementation popArgs2() returns (a1: real, a2: real)
{
    $sp := $sp - 1;
    a2 := $stack[$sp];
    $sp := $sp - 1;
    a1 := $stack[$sp];
}



procedure {:inline 1} fimport_2();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_2()
{
  var arg1: real;
  var arg2: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
}



procedure {:inline 1} popArgs3() returns (a1: real, a2: real, a3: real);
  requires $sp >= 3;
  modifies $sp, $stack;
  ensures $sp == old($sp) - 3;
  ensures 0 <= $sp;
  ensures (forall i: int :: $stack[i] == old($stack)[i]);
  ensures a1 == old($stack)[old($sp) - 3];
  ensures a2 == old($stack)[old($sp) - 2];
  ensures a3 == old($stack)[old($sp) - 1];



implementation popArgs3() returns (a1: real, a2: real, a3: real)
{
    $sp := $sp - 1;
    a3 := $stack[$sp];
    $sp := $sp - 1;
    a2 := $stack[$sp];
    $sp := $sp - 1;
    a1 := $stack[$sp];
}



procedure {:inline 1} fimport_3();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_3()
{
  var arg1: real;
  var arg2: real;
  var arg3: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 3;
    call arg1, arg2, arg3 := popArgs3();
}



procedure {:inline 1} popArgs1() returns (a1: real);
  requires $sp >= 1;
  modifies $sp, $stack;
  ensures $sp == old($sp) - 1;
  ensures 0 <= $sp;
  ensures (forall i: int :: $stack[i] == old($stack)[i]);
  ensures a1 == old($stack)[old($sp) - 1];



implementation popArgs1() returns (a1: real)
{
    $sp := $sp - 1;
    a1 := $stack[$sp];
}



procedure {:inline 1} fimport_4();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_4()
{
  var arg1: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 1;
    call arg1 := popArgs1();
}



procedure {:inline 1} fimport_5();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_5()
{
  var arg1: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 1;
    call arg1 := popArgs1();
}



procedure {:inline 1} fimport_6();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_6()
{
  var arg1: real;
  var arg2: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
}



procedure {:inline 1} fimport_7();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_7()
{
  var arg1: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 1;
    call arg1 := popArgs1();
}



procedure {:inline 1} fimport_8();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation fimport_8()
{
  var arg1: real;
  var arg2: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
}



procedure {:inline 1} func_0();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_0()
{
  var arg1: real;
  var arg2: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
    call push(arg1);
    call push(arg2);
    call push(32e0);
    call func_12();
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
}



procedure {:inline 1} func_1();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_1()
{
  var arg1: real;
  var arg2: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
    call push(arg1);
    call push(arg2);
    call push(32e0);
    call func_12();
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
}



procedure {:inline 1} func_2();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_2()
{
  var arg1: real;
  var arg2: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
    call push(arg1);
    call push(arg2);
    call push(32e0);
    call func_12();
    call push(0e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
}



procedure {:inline 1} func_3();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_3()
{
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    call fimport_1();
    call push(1e6);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 / $tmp1);
}



procedure {:inline 1} func_4();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_4()
{
  var arg1: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 1;
    call arg1 := popArgs1();
    call push(arg1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s64(idx);
    call push(int_to_real(load_i));
    call push(arg1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8;
    call load_i := mem_read_s64(idx);
    call push(int_to_real(load_i));
    call fimport_8();
}



procedure {:inline 1} func_5();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_5()
{
  var arg1: real;
  var arg2: real;
  var arg3: real;
  var loc1: real;
  var loc2: real;
  var loc3: real;
  var loc4: real;
  var loc5: real;
  var loc6: real;
  var loc7: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 3;
    call arg1, arg2, arg3 := popArgs3();
    loc1 := 0e0;
    loc2 := 0e0;
    loc3 := 0e0;
    loc4 := 0e0;
    loc5 := 0e0;
    loc6 := 0e0;
    loc7 := 0e0;
    call push(0e0);
    call push(0e0);
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(32e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call loc7 := popArgs1();
    call push(loc7);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(0e0);
    call loc4 := popArgs1();
    call push(59e0);
    call loc3 := popArgs1();
    call push(16e0);
    call loc2 := popArgs1();
    call push(0e0);
    call loc5 := popArgs1();

  label$1_start_2:
    call push(loc4);
    call push(6e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$6_end_8;
    }

    call push(25e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$5_end_7;
    }

    call push(loc1);
    call push(165e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc1 := popArgs1();
    goto label$4_end_6;

  label$6_end_8:
    call push(0e0);
    call loc6 := popArgs1();
    call push(loc4);
    call push(11e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 <= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$3_end_5;
    }

    goto label$2_end_4;

  label$5_end_7:
    call push(loc1);
    call push(208e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(0e0);
    call push(5e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    call popToTmp2();
    call popToTmp3();
    if (real_to_bool($tmp1))
    {
        call push($tmp3);
    }
    else
    {
        call push($tmp2);
    }

    call loc1 := popArgs1();

  label$4_end_6:
    call loc6 := popArgs1();

  label$3_end_5:
    call loc6 := popArgs1();

  label$2_end_4:
    call push(loc2);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc2 := popArgs1();
    call push(loc4);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc4 := popArgs1();
    call loc5 := popArgs1();
    call push(loc3);
    call push(-5e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call push(-6e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$1_start_2;
    }

    call push(loc5);
    call push(arg3);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$7_end_10;
    }

    call push(0e0);
    call loc4 := popArgs1();
    call push(59e0);
    call loc3 := popArgs1();
    call push(32e0);
    call loc2 := popArgs1();
    call push(0e0);
    call loc5 := popArgs1();

  label$8_start_11:
    call push(loc4);
    call push(4e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$13_end_17;
    }

    call push(25e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$12_end_16;
    }

    call push(loc1);
    call push(165e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc1 := popArgs1();
    goto label$11_end_15;

  label$13_end_17:
    call push(0e0);
    call loc6 := popArgs1();
    call push(loc4);
    call push(11e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 <= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$10_end_14;
    }

    goto label$9_end_13;

  label$12_end_16:
    call push(loc1);
    call push(208e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(0e0);
    call push(5e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    call popToTmp2();
    call popToTmp3();
    if (real_to_bool($tmp1))
    {
        call push($tmp3);
    }
    else
    {
        call push($tmp2);
    }

    call loc1 := popArgs1();

  label$11_end_15:
    call loc6 := popArgs1();

  label$10_end_14:
    call loc6 := popArgs1();

  label$9_end_13:
    call push(loc2);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc2 := popArgs1();
    call push(loc4);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc4 := popArgs1();
    call loc5 := popArgs1();
    call push(loc3);
    call push(-5e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call push(-6e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$8_start_11;
    }

    call push(loc5);
    call push(arg2);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 == $tmp1));
    call push(48e0);
    call fimport_2();

  label$7_end_10:
    call push(arg2);
    call push(arg1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 == $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$15_end_20;
    }

    call push(0e0);
    call loc4 := popArgs1();
    call push(59e0);
    call loc3 := popArgs1();
    call push(16e0);
    call loc2 := popArgs1();
    call push(0e0);
    call loc5 := popArgs1();

  label$16_start_21:
    call push(loc4);
    call push(6e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$21_end_27;
    }

    call push(25e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$20_end_26;
    }

    call push(loc1);
    call push(165e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc1 := popArgs1();
    goto label$19_end_25;

  label$21_end_27:
    call push(0e0);
    call loc6 := popArgs1();
    call push(loc4);
    call push(11e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 <= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$18_end_24;
    }

    goto label$17_end_23;

  label$20_end_26:
    call push(loc1);
    call push(208e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(0e0);
    call push(5e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    call popToTmp2();
    call popToTmp3();
    if (real_to_bool($tmp1))
    {
        call push($tmp3);
    }
    else
    {
        call push($tmp2);
    }

    call loc1 := popArgs1();

  label$19_end_25:
    call loc6 := popArgs1();

  label$18_end_24:
    call loc6 := popArgs1();

  label$17_end_23:
    call push(loc2);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc2 := popArgs1();
    call push(loc4);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc4 := popArgs1();
    call loc5 := popArgs1();
    call push(loc3);
    call push(-5e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call push(-6e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$16_start_21;
    }

    call push(loc5);
    call push(arg3);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$14_end_19;
    }

  label$15_end_20:
    call push(loc7);
    call push(arg1);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 24;
    store_i := real_to_int($tmp2);
    call mem_write_u64(idx, store_i);
    call push(arg3);
    call push(2147483647e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$14_end_19;
    }

    call push(loc7);
    call push(0e0);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 20;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc7);
    call push(1e0);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 16;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc7);
    call push(loc7);
    call popToTmp1();
    idx := real_to_int($tmp1) + 16;
    call load_i := mem_read_s64(idx);
    call push(int_to_real(load_i));
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8;
    store_i := real_to_int($tmp2);
    call mem_write_u64(idx, store_i);
    call push(loc7);
    call push(24e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(loc7);
    call push(8e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call func_7();
    call pop();

  label$14_end_19:
    call push(0e0);
    call push(loc7);
    call push(32e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
}



procedure {:inline 1} func_6();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_6()
{
  var arg1: real;
  var arg2: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
    call push(arg2);
    call fimport_7();
    call push(128e0);
    call fimport_5();
    call push(arg2);
    call fimport_4();
}



procedure {:inline 1} popDiscard3();
  requires $sp >= 3;
  modifies $sp;
  ensures $sp == old($sp) - 3;
  ensures 0 <= $sp;



implementation popDiscard3()
{
    $sp := $sp - 3;
}



procedure {:inline 1} func_7();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_7()
{
  var arg1: real;
  var arg2: real;
  var loc1: real;
  var loc2: real;
  var loc3: real;
  var loc4: real;
  var loc5: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
    loc1 := 0e0;
    loc2 := 0e0;
    loc3 := 0e0;
    loc4 := 0e0;
    loc5 := 0e0;
    call push(0e0);
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(16e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call loc5 := popArgs1();
    call push(0e0);
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(arg2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc1 := popArgs1();
    call push(arg2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc4 := popArgs1();
    call fimport_0();
    call arg2 := popArgs1();
    call push(arg2);
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$4_end_33;
    }

    call push(arg2);
    call push(513e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$3_end_32;
    }

    call push(arg2);
    call func_8();
    call loc3 := popArgs1();
    goto label$2_end_31;

  label$4_end_33:
    call push(0e0);
    call loc3 := popArgs1();
    goto label$1_end_30;

  label$3_end_32:
    call push(0e0);
    call push(loc3);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$2_end_31:
    call push(loc3);
    call push(arg2);
    call fimport_6();
    call pop();

  label$1_end_30:
    call push(loc5);
    call push(0e0);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8;
    store_i := real_to_int($tmp2);
    call mem_write_u64(idx, store_i);
    call push(arg2);
    call push(7e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call push(112e0);
    call fimport_2();
    call push(loc5);
    call push(8e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(loc3);
    call push(8e0);
    call fimport_3();
    call pop();
    call push(loc5);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8;
    call load_i := mem_read_s64(idx);
    call push(int_to_real(load_i));
    call loc2 := popArgs1();
    call push(arg2);
    call push(513e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$5_end_36;
    }

    call push(loc3);
    call func_11();

  label$5_end_36:
    call push(arg1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg2 := popArgs1();
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$6_end_37;
    }

    call push(arg2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(loc4);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc4 := popArgs1();

  label$6_end_37:
    call push(arg2);
    call push(loc2);
    call push(loc4);
    call popDiscard3();
    call push(0e0);
    call push(loc5);
    call push(16e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(1e0);
}



procedure {:inline 1} func_8();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_8()
{
  var arg1: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 1;
    call arg1 := popArgs1();
    call push(144e0);
    call push(arg1);
    call func_9();
}



procedure {:inline 1} func_9();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_9()
{
  var arg1: real;
  var arg2: real;
  var loc1: real;
  var loc2: real;
  var loc3: real;
  var loc4: real;
  var loc5: real;
  var loc6: real;
  var loc7: real;
  var loc8: real;
  var loc9: real;
  var loc10: real;
  var loc11: real;
  var loc12: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 2;
    call arg1, arg2 := popArgs2();
    loc1 := 0e0;
    loc2 := 0e0;
    loc3 := 0e0;
    loc4 := 0e0;
    loc5 := 0e0;
    loc6 := 0e0;
    loc7 := 0e0;
    loc8 := 0e0;
    loc9 := 0e0;
    loc10 := 0e0;
    loc11 := 0e0;
    loc12 := 0e0;
    call push(arg2);
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$1_end_38;
    }

    call push(arg1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8384;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc12 := popArgs1();
    call push(loc12);
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$2_end_39;
    }

    call push(16e0);
    call loc12 := popArgs1();
    call push(arg1);
    call push(8384e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(16e0);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$2_end_39:
    call push(arg2);
    call push(8e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc1 := popArgs1();
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call push(arg2);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call popToTmp3();
    if (real_to_bool($tmp1))
    {
        call push($tmp3);
    }
    else
    {
        call push($tmp2);
    }

    call loc1 := popArgs1();
    call push(arg1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8388;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc9 := popArgs1();
    call push(loc9);
    call push(loc12);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 >= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$5_end_44;
    }

    call push(arg1);
    call push(loc9);
    call push(12e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 * $tmp1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(8192e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg2 := popArgs1();
    call push(loc9);
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$6_end_46;
    }

    call push(arg1);
    call push(8196e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc12 := popArgs1();
    call push(loc12);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$6_end_46;
    }

    call push(arg2);
    call push(8192e0);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc12);
    call push(arg1);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$6_end_46:
    call push(loc1);
    call push(4e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc9 := popArgs1();

  label$7_start_48:
    call push(arg2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc12 := popArgs1();
    call push(loc12);
    call push(loc9);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(arg2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$8_end_50;
    }

    call push(arg2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 4;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(loc12);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc12 := popArgs1();
    call push(loc12);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(arg2);
    call push(8e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg2 := popArgs1();
    call push(arg2);
    call push(arg2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(loc9);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc12);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc12);
    call push(4e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg2 := popArgs1();
    call push(arg2);
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$4_end_43;
    }

  label$8_end_50:
    call push(arg1);
    call func_10();
    call arg2 := popArgs1();
    call push(arg2);
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$7_start_48;
    }

  label$5_end_44:
    call push(2147483647e0);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call loc3 := popArgs1();
    call push(arg1);
    call push(8392e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc10 := popArgs1();
    call push(arg1);
    call push(8384e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc11 := popArgs1();
    call push(arg1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8392;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc2 := popArgs1();
    call push(loc2);
    call loc12 := popArgs1();

  label$9_start_57:
    call push(arg1);
    call push(loc12);
    call push(12e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 * $tmp1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg2 := popArgs1();
    call push(arg2);
    call push(82e2);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(arg2);
    call push(8192e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc4 := popArgs1();
    call push(loc4);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 == $tmp1));
    call push(8544e0);
    call fimport_2();
    call push(arg2);
    call push(8196e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc5 := popArgs1();
    call push(loc5);
    call push(4e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc12 := popArgs1();

  label$10_start_62:
    call push(loc5);
    call push(loc4);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc6 := popArgs1();
    call arg2 := popArgs1();
    call push(loc8);
    call push(0e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$11_end_64;
    }

    call push(arg2);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 >= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$12_end_65;
    }

  label$13_start_66:
    call push(loc12);
    call push(arg2);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc9 := popArgs1();
    call push(loc9);
    call push(loc6);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 >= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$12_end_65;
    }

    call push(loc9);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc9 := popArgs1();
    call push(loc9);
    call push(0e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$12_end_65;
    }

    call push(arg2);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(4e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg2 := popArgs1();
    call push(arg2);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$13_start_66;
    }

  label$12_end_65:
    call push(loc7);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(arg2);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 <= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$14_end_71;
    }

    call push(loc12);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$14_end_71:
    call push(arg2);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 >= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$3_end_42;
    }

  label$11_end_64:
    call push(loc12);
    call push(arg2);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(4e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc12 := popArgs1();
    call push(loc12);
    call push(loc6);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$10_start_62;
    }

    call push(0e0);
    call arg2 := popArgs1();
    call push(loc10);
    call push(0e0);
    call push(loc10);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc12 := popArgs1();
    call push(loc12);
    call push(loc12);
    call push(loc11);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 == $tmp1));
    call popToTmp1();
    call popToTmp2();
    call popToTmp3();
    if (real_to_bool($tmp1))
    {
        call push($tmp3);
    }
    else
    {
        call push($tmp2);
    }

    call loc12 := popArgs1();
    call push(loc12);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc12);
    call push(loc2);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$9_start_57;
    }

  label$4_end_43:
    call push(arg2);
    goto func_exit_76;

  label$3_end_42:
    call push(loc7);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc12);
    goto func_exit_76;

  label$1_end_38:
    call push(0e0);

  func_exit_76:
}



procedure {:inline 1} func_10();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_10()
{
  var arg1: real;
  var loc1: real;
  var loc2: real;
  var loc3: real;
  var loc4: real;
  var loc5: real;
  var loc6: real;
  var loc7: real;
  var loc8: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 1;
    call arg1 := popArgs1();
    loc1 := 0e0;
    loc2 := 0e0;
    loc3 := 0e0;
    loc4 := 0e0;
    loc5 := 0e0;
    loc6 := 0e0;
    loc7 := 0e0;
    loc8 := 0e0;
    call push(arg1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8388;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc1 := popArgs1();
    call push(0e0);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8630;
    call load_i := mem_read_u8(idx);
    call push(int_to_real(load_i));
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$2_end_79;
    }

    call push(0e0);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8632;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc7 := popArgs1();
    goto label$1_end_78;

  label$2_end_79:
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call loc7 := popArgs1();
    call push(0e0);
    call push(1e0);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8630;
    store_i := real_to_int($tmp2);
    call mem_write_u8(idx, store_i);
    call push(0e0);
    call loc7 := popArgs1();
    call push(loc7);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8632;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$1_end_78:
    call push(loc7);
    call loc3 := popArgs1();
    call loc2 := popArgs1();
    call push(loc2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call loc8 := popArgs1();
    call push(loc8);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 <= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$6_end_84;
    }

    call push(loc2);
    call push(loc8);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call pop();
    call push(0e0);
    call loc8 := popArgs1();
    call push(loc2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$5_end_83;
    }

    call push(0e0);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8632;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc3 := popArgs1();

  label$6_end_84:
    call push(0e0);
    call loc8 := popArgs1();
    call push(0e0);
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8632;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc7);
    call push(0e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$5_end_83;
    }

    call push(arg1);
    call push(loc1);
    call push(12e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 * $tmp1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc2 := popArgs1();
    call push(loc7);
    call push(65536e0);
    call push(131072e0);
    call loc8 := popArgs1();
    call push(loc8);
    call push(64513e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call loc6 := popArgs1();
    call push(loc6);
    call popToTmp1();
    call popToTmp2();
    call popToTmp3();
    if (real_to_bool($tmp1))
    {
        call push($tmp3);
    }
    else
    {
        call push($tmp2);
    }

    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(loc8);
    call push(loc6);
    call popToTmp1();
    call popToTmp2();
    call popToTmp3();
    if (real_to_bool($tmp1))
    {
        call push($tmp3);
    }
    else
    {
        call push($tmp2);
    }

    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call push(loc7);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call loc7 := popArgs1();
    call push(0e0);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8630;
    call load_i := mem_read_u8(idx);
    call push(int_to_real(load_i));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$7_end_89;
    }

    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call loc3 := popArgs1();
    call push(0e0);
    call push(1e0);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8630;
    store_i := real_to_int($tmp2);
    call mem_write_u8(idx, store_i);
    call push(0e0);
    call loc3 := popArgs1();
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8632;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$7_end_89:
    call push(loc2);
    call push(8192e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc2 := popArgs1();
    call push(loc7);
    call push(0e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$4_end_82;
    }

    call push(loc3);
    call loc6 := popArgs1();
    call loc8 := popArgs1();
    call push(loc8);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call loc4 := popArgs1();
    call push(loc4);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 <= $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$8_end_91;
    }

    call push(loc8);
    call push(loc4);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call pop();
    call push(loc8);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$4_end_82;
    }

    call push(0e0);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8632;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc6 := popArgs1();

  label$8_end_91:
    call push(0e0);
    call push(loc6);
    call push(loc5);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8632;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc3);
    call push(-1e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 == $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$4_end_82;
    }

    call push(arg1);
    call push(loc1);
    call push(12e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 * $tmp1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc1 := popArgs1();
    call push(loc1);
    call push(8196e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc6 := popArgs1();
    call push(loc6);
    call push(loc2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc8 := popArgs1();
    call push(loc8);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(loc3);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 == $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$3_end_81;
    }

    call push(loc8);
    call push(loc1);
    call push(82e2);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc5 := popArgs1();
    call push(loc5);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc1 := popArgs1();
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 == $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$9_end_97;
    }

    call push(loc6);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc6 := popArgs1();
    call push(loc6);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc5);
    call push(loc2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc6);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$9_end_97:
    call push(arg1);
    call push(8388e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc2 := popArgs1();
    call push(loc2);
    call push(loc2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc2 := popArgs1();
    call push(loc2);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(arg1);
    call push(loc2);
    call push(12e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 * $tmp1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg1 := popArgs1();
    call push(arg1);
    call push(8196e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(arg1);
    call push(8192e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc8 := popArgs1();
    call push(loc8);
    call push(loc7);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$5_end_83:
    call push(loc8);
    goto func_exit_106;

  label$4_end_82:
    call push(loc2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc8 := popArgs1();
    call push(loc8);
    call push(arg1);
    call push(loc1);
    call push(12e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 * $tmp1);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call push(82e2);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc1 := popArgs1();
    call push(loc1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc7 := popArgs1();
    call push(loc7);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 == $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$10_end_107;
    }

    call push(loc3);
    call push(8196e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(loc7);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc1);
    call push(loc2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  label$10_end_107:
    call push(arg1);
    call push(arg1);
    call push(8388e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc7 := popArgs1();
    call push(loc7);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 8384;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc7);
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(0e0);
    goto func_exit_106;

  label$3_end_81:
    call push(loc2);
    call push(loc8);
    call push(loc7);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);
    call push(loc2);

  func_exit_106:
}



procedure {:inline 1} func_11();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_11()
{
  var arg1: real;
  var loc1: real;
  var loc2: real;
  var loc3: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 1;
    call arg1 := popArgs1();
    loc1 := 0e0;
    loc2 := 0e0;
    loc3 := 0e0;
    call push(arg1);
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$2_end_117;
    }

    call push(0e0);
    call popToTmp1();
    idx := real_to_int($tmp1) + 8528;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc2 := popArgs1();
    call push(loc2);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$2_end_117;
    }

    call push(8336e0);
    call loc3 := popArgs1();
    call push(loc2);
    call push(12e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 * $tmp1);
    call push(8336e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc1 := popArgs1();

  label$3_start_119:
    call push(loc3);
    call push(4e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call loc2 := popArgs1();
    call push(loc2);
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$2_end_117;
    }

    call push(loc2);
    call push(4e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(arg1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$4_end_122;
    }

    call push(loc2);
    call push(loc3);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_s32(idx);
    call push(int_to_real(load_i));
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call push(arg1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 > $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$1_end_116;
    }

  label$4_end_122:
    call push(loc3);
    call push(12e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call push(loc1);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 < $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$3_start_119;
    }

  label$2_end_117:
    goto func_exit_124;

  label$1_end_116:
    call push(arg1);
    call push(-4e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call loc3 := popArgs1();
    call push(loc3);
    call popToTmp2();
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    store_i := real_to_int($tmp2);
    call mem_write_u32(idx, store_i);

  func_exit_124:
}



procedure {:inline 1} func_12();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_12()
{
  var arg1: real;
  var arg2: real;
  var arg3: real;
  var loc1: real;
  var loc2: real;
  var loc3: real;
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume $sp >= 3;
    call arg1, arg2, arg3 := popArgs3();
    loc1 := 0e0;
    loc2 := 0e0;
    loc3 := 0e0;
    call push(0e0);
    call loc3 := popArgs1();
    call push(arg3);
    call popToTmp1();
    call push(bool_to_real($tmp1 == 0e0));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$1_end_126;
    }

  label$3_start_128:
    call push(arg1);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_u8(idx);
    call push(int_to_real(load_i));
    call loc1 := popArgs1();
    call push(loc1);
    call push(arg2);
    call popToTmp1();
    idx := real_to_int($tmp1) + 0;
    call load_i := mem_read_u8(idx);
    call push(int_to_real(load_i));
    call loc2 := popArgs1();
    call push(loc2);
    call popToTmp1();
    call popToTmp2();
    call push(bool_to_real($tmp2 != $tmp1));
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$2_end_127;
    }

    call push(arg2);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg2 := popArgs1();
    call push(arg1);
    call push(1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg1 := popArgs1();
    call push(arg3);
    call push(-1e0);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 + $tmp1);
    call arg3 := popArgs1();
    call push(arg3);
    call popToTmp1();
    if (real_to_bool($tmp1))
    {
        goto label$3_start_128;
    }

    goto label$1_end_126;

  label$2_end_127:
    call push(loc1);
    call push(loc2);
    call popToTmp1();
    call popToTmp2();
    call push($tmp2 - $tmp1);
    call loc3 := popArgs1();

  label$1_end_126:
    call push(loc3);
}



procedure {:inline 1} func_13();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation func_13()
{
  var entry_sp: int;
  var idx: int;
  var load_i: int;
  var store_i: int;

    entry_sp := $sp;
    assume false;
}



procedure {:inline 1} popDiscard1();
  requires $sp >= 1;
  modifies $sp;
  ensures $sp == old($sp) - 1;
  ensures 0 <= $sp;



implementation popDiscard1()
{
    $sp := $sp - 1;
}



procedure {:inline 1} CorralChoice_hello_target_hello_target();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation CorralChoice_hello_target_hello_target()
{
  var c: int;
  var argTmp: real;

    havoc c;
    assume 0 <= c && c < 23;
    if (c == 0)
    {
        call fimport_0();
        call popDiscard1();
    }
    else if (c == 1)
    {
        call fimport_1();
        call popDiscard1();
    }
    else if (c == 2)
    {
        havoc argTmp;
        call push(argTmp);
        havoc argTmp;
        call push(argTmp);
        call fimport_2();
    }
    else if (c == 3)
    {
        havoc argTmp;
        call push(argTmp);
        havoc argTmp;
        call push(argTmp);
        havoc argTmp;
        call push(argTmp);
        call fimport_3();
        call popDiscard1();
    }
    else if (c == 4)
    {
        havoc argTmp;
        call push(argTmp);
        call fimport_4();
    }
    else if (c == 5)
    {
        havoc argTmp;
        call push(argTmp);
        call fimport_5();
    }
    else if (c == 6)
    {
        havoc argTmp;
        call push(argTmp);
        havoc argTmp;
        call push(argTmp);
        call fimport_6();
        call popDiscard1();
    }
    else if (c == 7)
    {
        havoc argTmp;
        call push(argTmp);
        call fimport_7();
    }
    else if (c == 8)
    {
        havoc argTmp;
        call push(argTmp);
        havoc argTmp;
        call push(argTmp);
        call fimport_8();
    }
    else if (c == 9)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_0();
        call popDiscard1();
    }
    else if (c == 10)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_1();
        call popDiscard1();
    }
    else if (c == 11)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_2();
        call popDiscard1();
    }
    else if (c == 12)
    {
        call func_3();
        call popDiscard1();
    }
    else if (c == 13)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_4();
    }
    else if (c == 14)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_5();
    }
    else if (c == 15)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_6();
    }
    else if (c == 16)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_7();
        call popDiscard1();
    }
    else if (c == 17)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_8();
        call popDiscard1();
    }
    else if (c == 18)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_9();
        call popDiscard1();
    }
    else if (c == 19)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_10();
        call popDiscard1();
    }
    else if (c == 20)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_11();
    }
    else if (c == 21)
    {
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        havoc argTmp;
        assume argTmp == real(int(argTmp));
        call push(argTmp);
        call func_12();
        call popDiscard1();
    }
    else if (c == 22)
    {
        call func_13();
    }
}



procedure BoogieEntry_hello_target_hello_target();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation BoogieEntry_hello_target_hello_target()
{
  var c: int;
  var argTmp: real;

    call initGlobals();
    call InitRuntime();
    while (true)
      invariant 0 <= $sp;
    {
        havoc c;
        assume 0 <= c && c < 23;
        if (c == 0)
        {
            call fimport_0();
            call popDiscard1();
        }
        else if (c == 1)
        {
            call fimport_1();
            call popDiscard1();
        }
        else if (c == 2)
        {
            havoc argTmp;
            call push(argTmp);
            havoc argTmp;
            call push(argTmp);
            call fimport_2();
        }
        else if (c == 3)
        {
            havoc argTmp;
            call push(argTmp);
            havoc argTmp;
            call push(argTmp);
            havoc argTmp;
            call push(argTmp);
            call fimport_3();
            call popDiscard1();
        }
        else if (c == 4)
        {
            havoc argTmp;
            call push(argTmp);
            call fimport_4();
        }
        else if (c == 5)
        {
            havoc argTmp;
            call push(argTmp);
            call fimport_5();
        }
        else if (c == 6)
        {
            havoc argTmp;
            call push(argTmp);
            havoc argTmp;
            call push(argTmp);
            call fimport_6();
            call popDiscard1();
        }
        else if (c == 7)
        {
            havoc argTmp;
            call push(argTmp);
            call fimport_7();
        }
        else if (c == 8)
        {
            havoc argTmp;
            call push(argTmp);
            havoc argTmp;
            call push(argTmp);
            call fimport_8();
        }
        else if (c == 9)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_0();
            call popDiscard1();
        }
        else if (c == 10)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_1();
            call popDiscard1();
        }
        else if (c == 11)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_2();
            call popDiscard1();
        }
        else if (c == 12)
        {
            call func_3();
            call popDiscard1();
        }
        else if (c == 13)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_4();
        }
        else if (c == 14)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_5();
        }
        else if (c == 15)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_6();
        }
        else if (c == 16)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_7();
            call popDiscard1();
        }
        else if (c == 17)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_8();
            call popDiscard1();
        }
        else if (c == 18)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_9();
            call popDiscard1();
        }
        else if (c == 19)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_10();
            call popDiscard1();
        }
        else if (c == 20)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_11();
        }
        else if (c == 21)
        {
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            havoc argTmp;
            assume argTmp == real(int(argTmp));
            call push(argTmp);
            call func_12();
            call popDiscard1();
        }
        else if (c == 22)
        {
            call func_13();
        }
    }
}



procedure CorralEntry_hello_target_hello_target();
  modifies $tmp1, $tmp2, $tmp3, $sp, $stack, $mem;



implementation CorralEntry_hello_target_hello_target()
{
    call InitRuntime();
    call initGlobals();
    while (true)
    {
        call CorralChoice_hello_target_hello_target();
    }
}


